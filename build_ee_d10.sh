...
do
  ...
  then
    ...
    echo "! Debian 10 doesn't have Java 8! Android SDK requires Java 8!"
    if [ "${EE_UPDATE}" == "yes" ]
    then
      echo "Installing oldstable repos to get Java 8."
      sudo apt -y install software-properties-common &&
        sudo apt-add-repository 'deb http://security.debian.org/debian-security stretch/updates main' &&
        sudo apt update &&
        sudo apt -y install openjdk-8-jdk
    else
      echo "! noupdate passed, skipping oldstable/Java 8 install."
    fi
